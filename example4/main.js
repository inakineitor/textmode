...
function mainLoop() {
    ...
    const mergedName = [' '.repeat(nameWidth), ...nameInRows, ' '.repeat(nameWidth)].join('');
    const numberOfCharsRevealed = clamp(Math.floor((timeInSecondsSinceStart - 0.3) * 500), 0, mergedName.length);
    const revealedMask = generateRandomMask(mergedName.length, numberOfCharsRevealed);
    ...
    for (let i = 0; i < mergedName.length; i++) {
        ...
        if (char === ' ') {
            const adjacentChars = [
                mergedName[i - nameWidth - 1], mergedName[i - nameWidth], mergedName[i - nameWidth + 1],
                mergedName[i - 1], mergedName[i + 1],
                mergedName[i + nameWidth - 1], mergedName[i + nameWidth], mergedName[i + nameWidth + 1],
            ];
